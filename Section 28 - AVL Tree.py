...
def right_rotate(disbalanced_node):
    ...
    disbalanced_node.lef = disbalanced_node.left_child.right_child
    ...
    new_root.height = 1 + max(get_height(disbalanced_node.left_child), get_height(new_root.right_child))
    return new_root
...
def insert_node(root_node, node_value):
    ...
    if balance < -1 and node_value < node_value.right_child.data:
...
def delete_node(root_node, node_value):
    if not root_node:
        ...
        if root_node.left_child is None:
            ...
            root_node = None
            return temp
        temp = get_min_value(root_node.right_child)
